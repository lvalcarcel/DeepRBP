import sys
sys.path.insert(1,'/scratch/jsanchoz/envs/pytorch')
print('printing path')
print(sys.path)
import pandas as pd

path = '/scratch/jsanchoz/data_TcgaTargetGtex/validation_regulation/gencode_23/postar_3'
filename = path+'/human.txt'
...
mapping_dict = {
    ...
    'HepG2': {'morphology': 'epithelial', 'tissue': 'Liver_HepG2', 'disease': '', 'extra_info':None},
    ...
    'Huh7': {'morphology': 'epithelial', 'tissue': 'Liver_huh7', 'disease': 'Hepatocellular_Carcinoma', 
...
def process_postar3(filename, mapping_dict):
    ...
    df_tissue_description.to_csv(f'{path}/df_tissue_description.txt')
    df_postar3['tissue'] = df_postar3['raw_tissue'].apply(lambda x: mapping_dict[x]['tissue'])
    # 7) Create the tissue-specific Posta3 files
    #tissues = ["Pancreas_epith_carcinoma", "Pancreas_adenocarcinoma"] #comentar luego las siguientes dos lineas
    #[df_postar3[df_postar3.tissue == tissue].reset_index(drop='Index').to_csv(f'{path}/human_{tissue}.txt') for tissue in tissues]
    [df_postar3[df_postar3.tissue == tissue].reset_index(drop='Index').to_csv(f'{path}/human_{tissue}.txt') for tissue in df_postar3.tissue.unique()]
