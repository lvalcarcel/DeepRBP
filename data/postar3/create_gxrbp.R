...
path = "D:/Users/jsanchoz/data_TcgaTargetGtex/validation_regulation/gencode_23"
path = "C:/Users/jsanchoz/Downloads/validation_regulation/gencode_23"

tissues <- c("adrenal_gland", "Blastocyst_Embryo", "Bone_Marrow", "Brain", "Breast", "Colon", "Cortical", "Kidney", "Liver_HepG2", 
             "Liver_huh7", "myocardium", "Ovarian", "Pancreas", "Prostate", "Skin", "T_cell", "Uterus_Cervix")

#### 2) Load Data
# 2.1) <- EventsXtrans_gc23.RData: rdata que tiene 3 matrices (path1 x trsncritos, pat2 x trans, ref x trans), un vector 
# con el nombre de los transcritos y una lista con el splicing graph de cada gen.
load(paste0(path,"/create_exRBP/EventsXtrans_gc23.RData"))
#List with the information of the splicing graph of the genes. Returned by the funciotn EventDetectio_transcriptome
# SG_List <- EventsXtrans_gc23$SG_List

# 2.2) <- EventsFound_gencode23.txt: info de los eventos: posici�n, tipo de evento, nombre, id, etc...
# Path to eventsFound.txt with the information of all the events
pathtoeventstable <- paste0(path,"/create_exRBP/EventsFound_gencode23.txt")
...
# 2.3) <- EventsRegions <- : las regiones de los eventos
load(paste0(path,"/create_exRBP/Events_Regions_gc23_400nt.RData")) #GRseq3
...
for(tissue in tissues){
  ...
  human_txt <- read_delim(paste0(path,"/postar_3/human_",tissue, ".txt"), 
  ...
  path_save <- paste0(path,"/create_exRBP/result")
