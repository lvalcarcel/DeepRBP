...
def process_data_chunk(df_gene, df_trans, dictionary_data, getBM, path_data, select_genes):  
    ...
    ##### 6) List of RBPs:
    df_RBPs = pd.read_excel(f'{path_data}/external/Table_S2_list_RBPs_eyras.xlsx', skiprows=2) # List of splicing genes
    ...
    ##### 7) Drop the genes with just one transcript (NEW!!!!)
    getBM = pd.read_csv(f"{path_data}/processed/getBM_total.csv")
    ...
    if select_genes is not None: # select_genes == cancer_genes
        ...
        genes_s5_eyras = pd.read_excel(f'{path_data}/external/Table_S5_Cancer_splicing_gene_eyras.xlsx', skiprows=2)
        genes_s6_eyras = pd.read_excel(f'{path_data}/external/Table_S6_Cancer_gene_eyras.xlsx', skiprows=2)
        genes_cosmic = pd.read_csv(f'{path_data}/external/Table_Cancer_Gene_Census.tsv', sep="\t")
        ...
        getBM.to_csv(f'{path_data}/processed/getBM_reduced.csv')  
...
def create_datasets(path_data, chunksize, select_genes):
    ...
    for col_start in range(1, n_patients, chunksize+1): # for chunk:
...
def process_and_save_expression_data(path_data, id_samples, condition, path_model):
    """
    ...
    getBM = pd.read_csv(f'{path_model}/processed/getBM_total.csv')
    df_RBPs = pd.read_excel(f'{path_model}/external/Table_S2_list_RBPs_eyras.xlsx', skiprows=2) # List of splicing genes
    ids_RBPs = getBM[getBM['Gene_name'].isin(df_RBPs['HGNC symbol'].tolist())]['Gene_ID'].unique().tolist()
    ...
    getBM_mini = pd.read_csv(f'{path_model}/processed/getBM_reduced.csv', index_col=0)
